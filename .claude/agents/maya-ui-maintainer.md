...
- `plug-ins/MayaObjectBuilderTranslator.py` and `scripts/a3ob/mayabridge/translator.py` when translator registration is involved
...
- `initializePlugin()` and deregistration in `plug-ins/MayaObjectBuilder.py`

Responsibilities:
- Keep Python UI wrappers aligned with command flags and command names.
...
Default smoke commands (pure Python — no build step):
```bash
