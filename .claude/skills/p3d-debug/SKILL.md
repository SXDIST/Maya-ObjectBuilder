...
- N-gons are triangulated in `_export_mesh_lod()` via Maya polygon triangles.
...
- Export reads Maya's current active UV set through `MFnMesh.getUVs()`.
...
1. Reproduce with the smallest workflow: `tests/python/test_p3d_roundtrip.py` for binary format, `mayapy p3d_workflow.py` for Maya integration.
